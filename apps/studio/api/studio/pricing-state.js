...
module.exports = async function handler(req, res) {
  ...
  try {
    const result = await admin(req, `/admin/studio/pricing?product_id=${encodeURIComponent(productId)}`)
    if (result.unauthorized) return json(res, 401, { message: 'Unauthorized' })
    if (!result.response) return json(res, 502, { message: 'Pricing state service unavailable' })
    return json(res, result.response.status, result.payload || {})
  } catch (error) {
    console.error('studio pricing state failed', error)
    return json(res, 502, { message: 'Pricing state service unavailable' })
  }
}
