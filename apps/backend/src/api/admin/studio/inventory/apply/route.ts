...
function domainError(error: unknown) {
  ...
  if (message === "stale_draft") {
    return {
      status: 409,
      code: "stale_draft",
      message: "This draft changed in another session. Reload it before applying stock.",
    }
  }
  if (message === "stale_inventory_plan") {
    return {
      status: 409,
      code: "stale_inventory_plan",
      message: "The live inventory state changed after review. Refresh the stock plan before applying it.",
    }
  }
  ...
  if (message.includes("variant_graph_required") || message.includes("variants_required")) {
    return {
      ...
      message: "Build the product choices before applying stock.",
    }
  }
...
export const POST = async (
  ...
  try {
    const plan = await locking.execute(
      `coquette-studio-inventory:${productId}`,
      async () =>
        applyStudioInventoryPlan(
          req.scope,
          productId,
