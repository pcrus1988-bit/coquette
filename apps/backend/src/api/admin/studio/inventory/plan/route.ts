...
function domainError(error: unknown) {
  ...
  if (message.includes("variant_graph_required")) {
    return { status: 409, code: "variant_graph_required", message: "Build the product choices before reviewing stock." }
  }
  if (message.includes("not_a_draft")) {
    return { status: 409, code: "not_a_draft", message: "Stock in this flow is available only while the product remains unpublished." }
  }
