...
export default async function storageSmoke({ container }: ExecArgs) {
  ...
  if (missing.length) {
    throw unexpectedState(
      `Storage smoke cannot run; missing ${missing.join(", ")}`
    )
  }
  ...
  if (!uploaded?.url) {
    throw unexpectedState(
      "Storage smoke upload completed without a public file URL"
    )
  }
  ...
  if (
    ...
    throw unexpectedState(
      "Storage smoke public read did not return the uploaded PNG"
    )
  }
